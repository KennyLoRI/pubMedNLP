...
def get_doc_embeddings(doc_batch, tokenizer, model):
    encoded_input = tokenizer(doc_batch, padding=True, truncation=True, return_tensors='pt')
...
def prepare_write_output(combined_docs, embeddings):
    ...
    for combined_doc, embedding in zip(combined_docs, embeddings):
        row = {"combined_doc": combined_doc, "embedding": embedding}
        rows.append(row)
    return rows
...
tokenizer = AutoTokenizer.from_pretrained('pritamdeka/S-PubMedBert-MS-MARCO', model_max_length=512)

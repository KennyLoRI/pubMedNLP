...
Proclaimed to be the „next generation of search engines“ Question-answering systems were noted to show promising potential already in 2003 [^18]. Early systems were restricted to certain question types such as definitional questions (i.e., “What is X?”) questions following a specific question taxonomy[^19]. Such early systems employed a complex assembly of different linguistic methodologies such as inferring logic forms of sentences from dependency relations, question classification and pattern matching [^19]. The rise of large general-purpose language models, which store explicit knowledge in their parameters, functioning like a database and a text generator in one system made many of these complex interacting components obsolete, leading to a revolution in Natural Language Processing [^14]. Consequently, applying the performance of these LLMs to applications in the medical domain is a field of increasing exploration [^20]. But yet, resource limitations in retraining LLMs with recent information, the sensible and non-shareable information natural to the field, persisting hallucination problems as well the inability of LLMs to provide references for its answers restrict a wider adooption of these models [^20]. Retrieval augmented question-answering systems show promising potential to lead to more reliable, up-to-date and transparent results in knowledge-intensive tasks [^16]. Rirst research in this direction has been proposed [^21] [^22]. Cyril et al [^21] proposed Almanac, a typical RAG system based on a vector database, that stores obtained scientific medical literature embedded via text-embedding-ada-002 from OpenAI.

TODO: Give overview over Almanac and clinfio.ai and highlight where they differ and especially where our model is different. Most likely our stand-alone-criteria is: it can run locally, at no cost, based solely on opensource software. 

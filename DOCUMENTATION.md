...
- Text Generation (context preparation, llama2 integration, Prompting, answer generation, postprocessing)
...
Contrary to the systems discussed above, we demonstrate the ability to build a lightweight medical question-answering system with local runtime capacity based on exclusively open-source software. In other words, we demonstrate the possibility of creating a well-performing system for answering knowledge-intense medical questions without the need to share possibly sensible patient data with a third party through API calls. While this is not strictly relevant for PubMed-based applications, it highlights the way towards other medical question-answering applications, where data confidentiality might be more important. In doing so, we highlight several limitations of currently existing state-of-the-art technologies that may inform future work to further close the nevertheless persisting gap between performance and data privacy. 
...
For document retrieval, the vector database [ChromaDB](https://docs.trychroma.com/) was used. Early in the project, the database [elasticsearch](https://www.elastic.co/elasticsearch) which also implements vector search was considered, but quickly dropped, as [ChromaDB](https://docs.trychroma.com/) is directly integrated into [LangChain](https://python.langchain.com/docs/get_started/introduction), the main framework used for implementing the chat application. This direct integration made the use of Chroma very accessible and easy out of the box. Each inserted embedding was added with a document text, which gets retrieved when searching for the most appropriate embedding, and optionally metadata, which can be used to narrow down the search space with filters like previously mentioned.
...
Our ensemble retriever combines dense retrieval (based on either cosine similarity or max marginal relevance) with BM25, a term-frequency-based retrieval operation specifically suited for providing exact term-based matches. It has been shown that Okapi BM25 can perform worse than some alternatives such as BM25L, when confronted with longer documents [^6]. Nevertheless, we Okapi Bm25 was chosen since with an average of 182 tokens and 6.3 sentences per abstract which again were split into 2.7 paragraphs per abstract, neither the abstracts themselves and especially not the chunked paragraphs fall into the category of "very long documents" as Yuanhua et. al characterized the problematic cases. 

The underlying idea of running both retrievers in parallel is to ensure that both exact term-based relevance, as well as context-based relevance, are captured and later combined via the reciprocal fusion rank. The BM25 score for a document D with N documents given a query Q consisting of $q_i$ terms with i = 1, ..., n is:   

$$\text{BM25}(Q, D) = \sum^n_{i \in Q} IDF(q_i) \cdot \frac{{tf(q_i,D)}}{{tf(q_i, D) + k_{1} \cdot (1 - b + b \cdot \frac{{\text{docLength}}}{{\text{avgDocLength}}})}}$$

Where:
- $tf(q_i,d)=log_{10}(count(q_i,d)+1)$ measures the term frequency of term $\(i\)$ in the document d.
- $IDF(q_i) = log_{10}(\frac{N}{df_i})$ is the inverse document frequency weight of the term $q_i$.
- $df_i$ is the number of documents the term $q_i$ occurs in
...
The Reciprocal Fusion Rankformula combines rankings from the dense retrieval strategy and BM25. Given a set D of documents to be ranked and a set of Rankings R (with cardinality 2 in our case), the RRF score is determined by[^33]:

$$RRF(d \in D) = \sum_{r \in R}\frac{1}{k+r(d)}$$

Out of this eventually ranked set of retrieved documents the `top_k` are extracted.

Since  medical terminology can pose various challenges, no retrieval strategy exhibits a clear conceptual superiority apriori, although intuition suggested that a combination of context and specificity is possibly best suited to the wide-ranging and oftentimes critical queries in the medical domain [^7]. To determine the retriever strategy which meets the demands of the application scenario, a specific retriever evaluation was conducted based on a diverse manually annotated validation set. Details follow in the evaluation section.

### Obtaining LLM output
#### Prompting strategies
Once the `top_k` documents are extracted, they are formatted together with the initial (possibly corrected) query into a prompt that is inputted into our model for text generation. Due to the usage of a quantized model and the accompanying limitations which became evident in the first runs of the pipeline, a relatively short and precise prompt was selected. While this selected prompt was fixed during evaluation to restrict the amount of grid search combinations, we tested the impact of including  abstract information only versus also including metadata information into the input prompt as previously mentioned. This was only necessary due to the limitations of the model employed since it was not evident whether it could handle the increasing complexity of the input prompt with the metadata included. Further details follow in the section that discusses the experimental set-up of the study. 

#### Text generation
For text generation, we applied Meta's Llama2 with 7 billion parameters - the smallest version possible [^30]. To deploy and execute it, we utilized the open-source [llama.cpp library](https://python.langchain.com/docs/integrations/llms/llamacpp) via a LangChain integration, which enables easy-to-use and flexible Llama model integrations with one key advantage crucial to our project. Written in C++, llama.cpp enables running LLaMA models on resource-constrained devices by reducing the models’ memory usage via integer quantisation and other ways of efficient memory management native to its C++ implementation. This reduces the time necessary for generating an answer on a GPU-equipped machine from a few minutes to a few seconds, with even CPU-only execution being feasible (although significantly slower). Besides the ability to deploy Llama2 architectures efficiently with the help llama.cpp, Llama2 models are optimized for dialogue use-case scenarios and outperformed most of the existing state-of-the-art models in question-answering relevant tasks[^35]. However, the capability for local execution through integer quantisation, results in performance drawbacks. To balance model size with quality loss, we chose a 5-bit quantisation as recommended [here](https://huggingface.co/TheBloke/Llama-2-7B-Chat-GGUF).
...
Considering the timeframe of our extracted abstracts, it is not surprising that nearly all articles have an attributed publishing year between 2013 and 2023 as was demanded by the inclusion criteria. The very few articles that fall into the categories of 2024 and 2012 can most likely be traced back to inaccuracies in the Entrez API or Pubmed itself. Within the time range of 2013/01/01 and 2023/11/01, the retrieved data exhibits a heavily left-skewed publication year distribution. As can be seen in Figure 3, a possible explanation for this increased publication growth is the general rise of research in "artificial intelligence" (count:30176) which is the most common bigram among all abstracts, followed by "machine learning", "deep learning", "intelligence, ai" and "neural, network(s)". 
Nevertheless, the dataset goes beyond the scope of artificial intelligence research. Aside from typical academic bigrams such as "present study", biomedical-related bigrams such as "decision making" (count: 7019), "health, care" (count: 5563), "physical, activity" (count: 4698) or even specific vocabulary such as "autism, spectrum" (count: 2132) or "computed, tomography" demonstrate the variety of the retrieved abstracts. Consequently, the dataset is suitable to represent a knowledge base for a RAG system in the medical domain. 


### Evaluation data set
Existing evaluation datasets such as PubmedQA [^23] were inapplicable as the questions contained went beyond the scope of the inclusion criteria. To obtain an alternative dataset for evaluation, three different approaches were possible: a manual, a semi-automated and a fully automated approach. All three were based on different random subsets of the abstracts retrieved in the data extraction phase of the pipeline. Using this subset both the semi-automatic as well as the manual approach contained a stage where for each abstract one question was manually created, which a human should be able to answer given the corresponding abstract as a context. The automated approach instead, fed the abstract together with a task description into [Gemini](https://gemini.google.com/), instructing it to automatically create a question of a randomly varying question types based on a provided abstract. 

In the second stage of the dataset generation phase, the manual approach included the process of manually answering each previously created answer, thus serving as human-generated ground truth. Both the fully-automated as well as the semi-automated approach encompassed prompting GPT 3.5 to generate an answer to the given question based on the corresponding abstract as a context.
Although substantially faster, the fully automated approach yielded poor question-answer pairs as determined in a qualitative screening procedure. Out of 25 generated question-answer pairs only 6 passed the quality checks of encompassing: 
a) answerableness by a human with the correct context
b) meaningfulness of the question
c) correctness of the answer

Given these reliability issues, the fully automated approach was abandoned. Both the manual as well as the semi-automated approach led to adequate pairs leading to one fully human and one partly LLM-generated data set on which our system can be compared. 

![Figure 3](project_docs/sa_man_question_typs.png)
After quality checks, the resulting semi-automated dataset consisted of 53 question-answer pairs of which the majority were factoid (beginning with "what" or "which") (14), followed by yes/no question-answer pairs (9), comparative (8), how (6) and open-ended (6) questions. 
The manually generated question dataset contains 60  question-answer pairs, of which the majority were factoid (26) and descriptive (7) questions, followed by open-ended (6), yes/no (6), how (6), comparative (4), hypothetical (4) and multiple-choice (1) questions. Consequently, the total dataset consisted of 113 question-answer pairs for evaluation, assessing different difficulties and question types, with a focus on factoid questions. Especially in the manually generated dataset, emphasis was put on diversity within each of the mentioned categories. Consequently within each category different capacities were necessary to yield high-performance levels, which included but were not limited to: inferring causal relationships and extrapolating correct scenarios.

### Validation and Evaluation Method
To reproducibly determine the best possible system with the given components, we ran a grid-search validation phase that tests various combinations of parameters end-to-end, computing their performance on 47 questions, equaling about ~40% of the whole available data, sampled from all possible categories of the question. The remaining 66 questions were used as a test set for evaluation. 
In the subsequent evaluation phase, we evaluated the best parameter combination identified in the validation phase once with dense-only retrieval and once with ensemble retrieval. This was necessary as the ensemble retriever could not be included in the validation gridsearch, as each initialization of this type of retriever can take up to 5 minutes, due to a costly workaround necessary as ChromaDB does not integrate with the required BM25 retriever yet. As a result, the best combination determined in the validation will be compared with the same combination except the retrieval strategy being replaced with the ensemble retrieval, to be able to still evaluate the performance of using an ensemble of retrievers compared to using only one retriever. Besides evaluating the system end-to-end, we also evaluated the best parameter combination with all possible retrieval strategies on the evaluation dataset to specifically assess the impact of the retriever on the whole system.

The performance for end-to-end validation and evaluation was measured by BLEU, ROUGE, BERTScore and BleuRT scores. The retriever evaluation performance was measured by recall, where for one question-answer example the recall is either 1 or 0, depending on whether one of the top k retrieved sources is the gold source, where the gold source is always exactly one abstract.

We include BLEU and ROUGE in our validation and evaluation metrics due to historical reasons, as they mainly measure lexical overlap, which can be useful for translation tasks but are very misleading when the phrasing of answers is different but essentially correct. The most important scores are BERTScore and BleuRT, which are neural-based methods and better suited to measure the similarity between generated answers and gold answers [^29]. To rank the combinations we define the metric 'Weighted Score' as a combination of the previously mentioned scores which can be computed in the following way:

$$\text{Weighted Score} = \text{BLEU} \cdot 0.1 + \text{ROUGE} \cdot 0.1 + \text{BERTScore} \cdot 0.4 + \text{BleuRT} \cdot 0.4$$

The metric 'Weighted Score' is a combination of multiple other metrics and better represents the significance of the different metrics, making it a robust and suitable measure to compare the results.
...
For the parameter `temperature` the values 0 and 0.5 were used, with higher values flattening the sampling probability of tokens, diversifying the generated text, but not necessarily improving quality [^31]. The parameter `abstract_only` controls whether metadata information is included in the prompt for the LLM, where the value `False` includes additional information and `True` only appends the text of the corresponding abstract or paragraph to the prompt. The parameter `metadata_strategy` determines if intentions from the user query are extracted which subsequently are used as filters to narrow down the search space. Originally the options `parser`, `none` and `llm_detection` were used, but using the LLM to identify authors or paper titles in the user query showed very low reliability and the option was consequently removed for validation. The parameter `granularity` controls which type of documents are extracted from the ChromaDB database. With the option `abstracts`, embeddings including the abstract text combined with metadata are used and the corresponding documents are extracted. With the option `paragraphs`, paragraph embeddings are used, and the corresponding documents including paragraph text and metadata are extracted. Depending on which `granularity` option is set, the parameter `top_k` determines if 2 or 3 abstract-based documents or 4 or 6 paragraph-based documents are retrieved from the vector database. The number of documents extracted for paragraphs is twice as high as for abstract-based documents, as each abstract roughly takes twice the amount of space as a paragraph-based document since there are 2.7 paragraphs per abstract on average. Finally the parameter `retrieval_strategy` determines the used retrieval approach. The option `ensemble_retriever`, which uses a BM25 retriever combined with another retrieval strategy, was not included in the validation grid due to limited computational resources, as the initialization of the ensemble retriever can take significant time due to the limitations explained above.
...
In general, we would have liked to perform a more extensive validation with more parameters, more diversity in the values as well as a complete cross-validation with multiple splits of the complete dataset with multiple runs for each part, as the generated answers are stochastic. However due to time constraints and the available computational resources a thorough and complete validation was not possible and had to be severely limited, so that the validation could be completed in a reasonable timeframe and without further computational resources.

For evaluation, the best combination of parameters from the validation was used. Additionally, another combination based on the best combination was compared, where the retriever was replaced with an ensemble retriever, to see if the performance of the system could be improved. Both combinations were then evaluated on the defined test set, and results with the selected metrics gathered on all different question types separately and all questions together as a whole.

Finally, an evaluation of all suggested retrieval strategies in this project was performed, to more precisely determine the impact of different retrieval options on the system as a whole. For this, the best combination of parameters with different retrieval strategies was evaluated and recall for each strategy was calculated. For each question-answer-source tuple, the recall can be either 1 or 0, depending on if one of the retrieved top k sources is the gold source.
...
For most combinations, the BLEU score was extremely low and ROUGE in general low but much higher, indicating that the LLM generated much longer answers than the gold answer, as BLEU roughly measures how much of the words generated by the LLM are included in the gold reference and ROUGE roughly measures how much of the gold reference appears in the generated answers. This means that our system generates answers which are included in the gold reference (higher ROUGE) but also many words not included in the gold reference (low BLEU). Also, for most combinations, BERTScore was generally higher than the BleuRT score, where BERTScore hovered from around ~74% to ~77% and BleuRT from ~37% to ~45%. As the weighted score mostly considers BERTScore and BleuRT, the final overall score hovers around ~46% to ~51%. 
...
Other than that, there are no clear winners in terms of parameters besides some indications. For example for the parameter `metadata_strategy` the MMR retrieval is the only present strategy in the top 5 combinations, but is also included in 2 of the 5 bottom combinations. One might conclude that MMR works particularly well with the granularity `abstracts`, but falls similar to the cosine-similarity-based retrieval short with the granularity `paragraphs`. Nonetheless, the cosine-similarity strategy takes all of the bottom 3 spots, which strongly suggests that MMR might work better than pure cosine-similarity in terms of retrieval.

Another interesting observation looking at the full results is that the parameter `temperature` with the value 0.5 and the parameter `abstract_only` with the value 'True' perform the highest, but also the lowest in terms of all combinations. Due to the higher variance of the generated answers resulting from higher temperature, the overall score might also be influenced by this variance, making results with high temperature very good or suboptimal, and results with low temperature more mediocre. To be able to make a more accurate statement, each combination would need to be validated multiple times, as the generated answers of the model could be different each time, and then combine the score of the multiple validations. But as mentioned before, the severely limited computational capacity and time constraints did not allow for more elaborate experiments.

#### End-to-End Evaluation Results

For a baseline to compare our results to, the questions were input manually into ChatGPT without context and the answer was saved as the prediction. The noted prediction was then compared with the gold answer. The baseline results for the different question types ordered by 'Weighted Score' and the overall results can be seen in the following table:
...
To further analyze the impact of the retrieval in general, all strategies are compared with each other in the following retriever evaluation.

#### Retriever Evaluation Results

For the retriever evaluation, the best combination was evaluated, once for every possible retrieval strategy. Then, the overall recall of each retriever was determined. This way, the impact of the retriever on the whole system can be evaluated specificaly. For each question-answer-source pair the top k retrieved sources from the vector database were compared with the gold source, the source of the question and answer. If any of the retrieved sources match with the gold source, the recall for this single tuple is considered as 1. Otherwise, the recall for this tuple is 0. The overall recall was computed by taking the mean of all recalls of the tuples. Below the results of different retrieval strategies can be seen:
...
The results further fortify the impression already gained in the end-to-end evaluation. The ensemble retriever performs superior in retrieving the correct document more often than other retrieval strategies, in this case, cosine-similarity or MMR-only retrieval: With the BM25 and MMR combination in 61.29% of the questions, the gold source was in the top k retrieved documents. This impacts the whole system and as a result also improves answer generating of the LLM, which explains the higher scores for the question type evaluation when using an ensemble retrieval strategy.
...
### Acknowledgements
We thank Prof. Gertz for this exceptionally engaging course and Satya for her valuable time to give us very helpful advice.
...
[^33]: Cormack, Gordon V., Charles LA Clarke, and Stefan Buettcher. "Reciprocal rank fusion outperforms condorcet and individual rank learning methods." Proceedings of the 32nd international ACM SIGIR conference on Research and development in information retrieval. 2009.

[^34]: Rahutomo, Faisal, Teruaki Kitasuka, and Masayoshi Aritsugi. "Semantic cosine similarity." The 7th international student conference on advanced science and technology ICAST. Vol. 4. No. 1. 2012.

[^35]: Touvron, Hugo, et al. "Llama 2: Open foundation and fine-tuned chat models." arXiv preprint arXiv:2307.09288 (2023).

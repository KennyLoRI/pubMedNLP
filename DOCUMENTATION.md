...
Proclaimed to be the „next generation of search engines“ Question-answering systems were noted to show promising potential already in 2003 [^18]. Early systems were restricted to certain question types such as paragraph-level text for definitional questions (i.e., “What is X?”) or more a fixed generic question taxonomy[^19]. Such early systems employed a complex assembly of different linguistic methodologies such as inferring logic forms of sentences from dependency relations, question classification and pattern matching [^19]. The rise of large general purpose language models, which stores explicit knowledge in its parameters, functioning like database and a text generator in one system made many of these complex interacting components obsolete, leading to a revolution in Natural Language Processing [^14]. Consequently, applying the performance of these LLMs to applications in the medical domain is a field of increasing exploration [^20]. But yet, resource limitations in retraining LLMs with recent information, the sensible and non-shareable information dealt with in medicine, persisting hallucination problems as well the inability of LLMs to provide references for its answers restrict the direct and wide applicability of these models [^20]. Retrieval augmented question-answering systems exhibit promising potential to lead to more reliable, up-to-date and transparent results in knowledge-intensive tasks [^16]. Due to this potential, first research in this direction has been proposed [^21] [^22]. 

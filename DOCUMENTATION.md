...
In this project, we test the possibility of creating a lean RAG system containing state-of-the-art components, that solves knowledge-intensive question-answering tasks, meeting the high standards of the medical domain. To evaluate the performance of this system we created an evaluation set consisting of 53 semiautomatically and 60 manually crafted question-answer pairs. For evaluation we employed a two-staged grid search process, assessing the full pipeline end-to-end for over X combinations [TODO: Insert number of combinations] as well as different retrieval options. The scores of the resulting best-evaluated system emphasized the possibility of creating a relatively lightweight system that still yields decent performance on a wide range of question-answering tasks with varying difficulty.  
